...
userinit(void)
{
  ...
  //printf("[DEBUG] p->nice: %d, p->counter: %d\n", p->nice, p->counter); // PA4
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // There are some runnable processes left. 
    // Purpose of this check is if there is no runnable process, it must busy-wait.
    if(!no_runnable_proc){

      // Counter is exhausted. Start a new epoch, redistribute time slices.
      if(counter_exhausted){
        for(p = proc; p < &proc[NPROC]; p++){
          acquire(&p->lock);
          
          if(p->state == RUNNABLE)      // Runnable process
            p->counter = ((20 - (p->nice)) >> 2) + 1;
          else if(p->state == SLEEPING) // Blocked process
